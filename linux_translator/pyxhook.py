"""Source - https://github.com/hiamandeep/py-keylogger"""
...
class HookManager(threading.Thread):
    """This is the main class. Instantiate it, and you can hand it
    KeyDown and KeyUp (functions in your own code) which execute to
    parse the pyxhookkeyevent class that is returned.

    This simply takes these two values for now:
    KeyDown = The function to execute when a key is pressed,
    if it returns anything. It hands the function an argument that
    is the pyxhookkeyevent class.
    KeyUp = The function to execute when a key is released,
    if it returns anything. It hands the function an argument that is
    ...
    def __init__(self):
        ...
        self.shiftablechar = re.compile(
            '^[a-z0-9]$|^minus$|^equal$|^bracketleft$|^bracketright$|'
            '^semicolon$|^backslash$|^apostrophe$|^comma$|^period$|'
            '^slash$|^grave$'
        )
    ...
    def run(self):
        ...
        # record_disable_context, while calling the callback
    ...
    def processevents(self, reply):
        ...
        while len(data):
            ...
            if event.type == X.KeyPress:
                ...
                # use mouse moves to record mouse position,
                # since press and release events
    ...
    # printable chars rather than being the correct inverse
    # of XK.string_to_keysym()
...
class pyxhookkeyevent:
    ...
    KeyID = This is just False for now. Under windows, it is the Virtual
    Key Code, but that's a windows-only thing.
    ScanCode = Please don't use this. It differs for pretty much every
    type of keyboard. X11 abstracts this information anyway.
    MessageName = "key down", "key up".
    """
    
    def __init__(
            self, Window, WindowName, WindowProcName, Key, Ascii,
